...
\vfill
\begin{center}
\qrcode[height=0.1\textwidth]{https://cosima.org.au}\\
\href{https://cosima.org.au}{\textsf{\textbf{cosima.org.au}}}
\end{center}
...
\begin{enumerate}
\item This is a technical report to document the configuration and performance of the ACCESS-OM2 suite of models at 1$^\circ$, 0.25$^\circ$ and 0.1$^\circ$ horizontal resolution (\url{https://cosima.org.au/index.php/models/}), intended to be a resource for the user community (e.g. COSIMA) and readily updated. This approach was partly inspired by \citet{Griffies2015a}.
...
\TODO{copy things from Nic's talk \url{https://cosima.org.au/wp-content/uploads/2018/06/COSIMA2018-Hannah.pdf}, Marshall's COSIMA 2018 workshop talk}
% done: Bluelink talk 2018, ARCCSS 2017 workshop poster, AMOS2018 talk, COSIMA 2018 workshop

\section{Introduction}
This technical report documents the ACCESS-OM2 ocean-sea ice model configurations at nominal horizontal resolutions of $1^\circ$, $0.25^\circ$ and $0.1^\circ$ developed by the Consortium for Ocean-Sea Ice Modelling in Australia (COSIMA, \url{https://cosima.org.au}).
...
The vertical grid is staggered, with vertical velocity points offset from tracer and horizontal velocity points.
...
\param{topog.nc} defines the t-cell bathymetry \param{ht}, and is used to define \param{kmt}, the number of wet cells in each t-cell column.
The depth at u-points \param{hu} is calculated internally as the minimum of the surrounding four \param{ht} values.
Similarly, the number of wet cells in each u-cell column \param{kmu} is calculated as the minimum of the surrounding four \param{kmt} values.
...
The along-isopycnal Redi tracer diffusion \citep{Redi1982} (also known as ``neutral diffusion'') in the two coarser configurations has a diffusivity that differs from GM (\mom{aredi_equal_agm}=false).
...
The isopycnal slope is multiplied by the product of the following two tapering functions when used to calculate neutral diffusion.
\mom{dm_taper} defaults to true, so we use the $\tanh$ tapering scheme of \citet{DanabasogluMcWilliams1995a} in regions of isopycnal slope exceeding \mom{smax}=0.01 in magnitude, with the taper applied over a slope width \mom{swidth}=0.0005 (these are both the default values).
\mom{neutral_eddy_depth} is true, so we also use tapering over an ``eddy depth'' based on the isopycnal slope and Rossby radius, or KPP (whichever is deeper), following \citet{LargeDanabasogluDoneyMcWilliams1997a}.
Diagnostics of the form \href{https://github.com/mom-ocean/MOM5/blob/6bdbdd4892543bbade921fa3224b2530d93c6f40/src/mom5/ocean_param/neutral/ocean_nphysicsC.F90#L1285-L1290}{\param{<tracer_name>_yflux_ndiffuse}} will output the neutral (i.e.~Redi) flux of any tracer in the y-direction.
Similarly the x-flux is available via \param{<tracer_name>_xflux_ndiffuse}.
See chapter~14 of \citet{Griffies2004a} for further explanation, and chapter~23 (particularly section~23.5.2) of \citet{Griffies2012a} for implementation details.
We use the \param{ocean_nphysicsC} formulation (\mom{use_nphysicsc}=true).
Relevant MOM5 source code files are \href{https://github.com/mom-ocean/MOM5/blob/master/src/mom5/ocean_param/neutral/ocean_nphysics_util.F90}{\param{ocean_nphysics_util.F90}} and \href{https://github.com/mom-ocean/MOM5/blob/master/src/mom5/ocean_param/neutral/ocean_nphysicsC.F90}{\param{ocean_nphysicsC.F90}}.
\WARNING{The MOM5 implementation of Redi diffusivity tapering has a couple of bugs which may cause dianeutral diffusion and large neutral diffusive flux convergences in regions of steep isopycnal slope --- see \url{https://github.com/mom-ocean/MOM5/issues/399}.}
...
Vertical diffusion of both tracers and momentum is parameterized using the K-profile parameterization \citep[KPP,][\mom{vert_mix_scheme}='kpp_mom4p1']{Large1994}, which governs mixing within the surface boundary layer, as well as interior convection (when the stratification is unstable), Richardson number-based shear instability (active mainly in the equatorial undercurrents), internal wave breaking (\mom{diff_cbt_iw} and \mom{visc_cbu_iw}),\TODO{check! these are both zero} and double-diffusion (\mom{double_diffusion}=true).
See \href{https://github.com/mom-ocean/MOM5/blob/6bdbdd4892543bbade921fa3224b2530d93c6f40/src/mom5/ocean_param/vertical/ocean_vert_kpp_mom4p1.F90#L964-L992}{\param{ocean_vert_kpp_mom4p1.F90}}.
% is KPP really used for all this? Isn't it just for the surface boundary layer??
...
The background vertical tracer diffusivity is zero in ACCESS-OM2-025 (\mom{j09_diffusivity} is unspecified, and the default is false).
Background vertical tracer diffusivity was also zero in the original version of ACCESS-OM2-01 in \cite{KissETAL2020a}, but the current version uses \SI{1e-6}{m^2s^{-1}} --- see section~\ref{0.1changes}.
...
and uses the pre-TEOS-10 freezing temperature (\mom{freezing_temp_preteos10})=true.
 See \href{https://github.com/pedrocol/basal_mom5-collaborative-project/issues/77#issuecomment-2138770602}{here} for more details on frazil.
...
cf. Andrew Roberts RASM cice namelist (Petra email 6 June 2018) \TODO{get permisssion}
...
See Andrew Roberts' comments on \citet{RobertsCraigMaslowskiOsinskiDuvivierHughesNijssenCassanoBrunke2015a} in email from Petra email 6 June 2018 --- inertial coupling can be essential for stability
...
We use \cice{kcatbound}=0, so the lower bounds of the thickness categories are 0, 0.6445072, 1.391433, 2.470179 and 4.567288\,m (from \param{NCAT} in CICE output files), 
...
There is an ongoing debate regarding the suitability of visco-plastic ice rheology, particularly to represent sea ice on fine scales \citep{Nye1973a, WeissSchulsonStern2007a, LindsayZhangRothrock2003a,KwokHunkeMaslowskiMenemenlisZhang2008a, GirardWeissMolinesBarnierBouillon2009a, Feltham2008a, DansereauWeissSaramitoLattes2016a, HutterLoschMenemenlis2018a, WeissDansereau2017a, HutterLoschMenemenlis2018a}. 
...
The \href{https://github.com/COSIMA/access-om2/releases/tag/GMD2019}{ACCESS-OM2 v1.0 configurations} used in \citet{KissETAL2020a} are forced with the JRA55-do v1.3 atmospheric reanalysis \citep[][\url{http://www.cesm.ucar.edu/events/wg-meetings/2018/presentations/omwg/kim.pdf}, \url{https://climate.mri-jma.go.jp/~htsujino/jra55do.html} and \url{http://www.clivar.org/omdp/japan2016}]{TsujinoETAL2018a, TsujinoUrakawaNakanoSmallKimYeagerDanabasogluSuzukiBamber2018a, Tsujino2015b, Tsujino2015a, Tsujino2016a, TsujinoUrakawaSuzukiKomuroSmallKimYeagerDanabasogluGriffies2016a, KimSmallYeagerDanabasogluTsujinoETAL2018a}.
JRA55-do is derived from the 55-year Japanese reanalysis \citep[JRA55,][\url{http://jra.kishou.go.jp/JRA-55/index_en.html}]{KobayashiOtaHaradaEbitaMoriyaOnodaOnogiKamahoriKobayashi2015a} with adjustments for driving ocean models. 
It has significantly improved bias, spatial and temporal resolution (55 km, 3-hourly), temporal extent (1958--2018), and dynamical self-consistency compared to the CORE dataset (200 km, 6-hourly, 1948--2009) used in many previous studies. We use recent calving and basal melt estimates from \citet{DepoorterBamberGriggsLenaertsLigtenbergBroekeMoholdt2013a}, which are spatially variable and somewhat larger than the uniform values in CORE \citep{TsujinoETAL2018a}. JRA55-do has also been regularly updated, allowing near-real-time experiments. The improved spatial resolution of wind is important for better representation of coastal polynyas \citep{StosselZhangVihma2011a, ZhangVihmaStosselUotila2015a}.
...
but is coarser and somewhat shorter than the ERA5 reanalysis (31\,km, 3-hourly, 1950--present): \url{https://confluence.ecmwf.int//pages/viewpage.action?pageId=74764925}.% https://www.ecmwf.int/en/forecasts/datasets/archive-datasets/reanalysis-datasets/era5
...
JRA55-do v1.3 river runoff is from \citet{SuzukiYamazakiTsujinoKomuroNakanoUrakawa2018a}; this is daily, interannually varying runoff from the JRA55 land surface model at 0.25$^\circ$ resolution, adjusted to match the observational estimates of \citet{DaiQianTrenberthMilliman2009a}.
...
Consequently \mom{runoff_insertion_thickness} and \mom{calving_insertion_thickness} are ignored, and the combined liquid (river) and solid (calving) discharge is inserted at the coast (or ice shelf edge) in the top \mom{river_insertion_thickness}=40\,m.
...
% and implemented it in \url{https://github.com/pedrocol/basal_mom5-collaborative-project}
...
Solid and liquid runoff are combined and the latent heat required to melt the ice is neglected. 
The diagnostic \param{sfc_hflux_from_runoff} is the heat flux (relative to 0\, C) from liquid river runoff - see \url{https://github.com/COSIMA/access-om2/issues/139#issuecomment-639278547}.
The temperature this corresponds to is given by the diagnostic \param{temp_runoff} --- see
\url{https://github.com/mom-ocean/MOM5/blob/d7ba13a/src/mom5/ocean_core/ocean_sbc.F90#L2434}.
\mom{land_model_heat_fluxes} defaults to false --- see \url{https://github.com/mom-ocean/MOM5/blob/d7ba13a/src/mom5/ocean_core/ocean_sbc.F90#L3832}.
Also see \cite{Griffies2019a}.
...
Repeat-year forcing (RYF) \citep{StewartKimUrakawaHoggYeagerTsujinoNakanoKissDanabasoglu2020a} takes a 12-month (not necessarily calendar-year) forcing from JRA55-do and repeats it indefinitely.
...
RYF forcing data is generated by \url{https://github.com/COSIMA/make_ryf}.
...
The new IAF configurations use JRA55-do 1.4.0 from
...
Configurations using JRA55-do v1.5.0 are also available, including forcing up to the end of 2023: \url{https://github.com/COSIMA/access-om2/issues/247}.
...
The only restoring we apply is to surface salinity, which is restored to \param{salt_sfc_restore.nc}.
This is based on the 0.25$^\circ$ World Ocean Atlas 2013 v2 monthly climatology (\url{https://www.nodc.noaa.gov/OC5/woa13/}) decav product used for the initial conditions (section~\ref{S:initial_conditions}); land values are filled with a conjugate gradient method, then the data at 0m and 10m is averaged, then horizontally smoothed to deal with spurious discontinuities and noise in WOA13v2, and smoothly interpolated to the model grid (see \url{https://github.com/COSIMA/access-om2/issues/74#issuecomment-459310692} and \url{https://github.com/COSIMA/access-om2/blob/master/tools/make_salt_sfc_restore.py}  and \url{https://github.com/COSIMA/access-om2/issues/103}). % ncdump -h salt_sfc_restore.nc gives: This salinity distribution is based on WOA13v2 where the land values are filled in with a conjugate gradient method. It is a simple average of land-filled salinity data at the depth of 0 and 10 meters.
...
Note that the MOM diagnostics \param{pme_river} and \param{total_ocean_pme_river} report the water fluxes into the ocean model component (not the total ocean-sea ice system), so are non-zero due to water exchange with sea ice due to freezing and melting.
...
\href{https://github.com/mom-ocean/MOM5/blob/730f0bf56f64fad0d9abcfa973b7348a89c175c3/src/mom5/ocean_core/ocean_sbc.F90#L5953}{\param{total_ocean_pme_river}} diagnostics.
Note that \mom{zero_net_water_couple_restore} and \mom{zero_net_water_restore} are ignored, because \mom{salt_restore_as_salt_flux} and \mom{use_waterflux} are both true.
More details are available in \url{https://docs.google.com/document/d/1gdfU-yDVOl2O6c4LoAno2rNHhWW7LqfI1_4bIS6GPSI}.
...
%andy 9:39 PM
%OK, IÕm not convinced by this. I can see from the code that pme+river is balanced by zero_net_water_coupler. And I can see ice which is formed and then one day melts again will get zeroed out in the long run. But I canÕt see what happens to precip which lands on the sea ice (as snow or water). Is this passed into the ocean as melt or as precip? If the former, then how can it be balanced? If the latter, I canÕt see how that is achieved É
...
The model sea ice has a fixed bulk salinity; we use the MOM5 default \mom{ice_salt_concentration} = 0.005 kg salt / kg ice, matching the CICE5 parameter \cice{ice_ref_salinity} defined in drivers/auscom/ice_constants.F90 \citep[but differing from the ACCESS-OM value of 0.004 kg salt / kg ice, ][table~2]{BiMarslandUotilaOFarrellFiedlerSullivanGriffiesZhouHirst2013a}. 
...
We use \citet{LargeYeager2004a} turbulent flux bulk formulas (\cice{ncar_ocean_flux}=true, \cice{ncar_ocean_flux_orig}=false), for the air-ocean drag, evaporation and sensible heat flux \citep[eq.\ 4--10 in ][]{LargeYeager2004a} based on model SST and prescribed JRA55-do near-surface wind, humidity and air temperature.
...
To prevent high concentrations of runoff a method for spatially variable conservative flux limiting has been implemented. Values are set in \href{https://github.com/ACCESS-NRI/access-om2-configs/blob/release-01deg_jra55_iaf/atmosphere/atm.nml}{\param{atmosphere/atm.nml}} by \yatm{runoff_caps} in units of kg\,m$^{-2}$\,s$^{-1}$; $i$-index start and end regions are set by \yatm{runoff_caps_is} and \yatm{runoff_caps_ie}, and $j$-index start and end regions are set by \yatm{runoff_caps_js} and \yatm{runoff_caps_je}.
The number of caps actually used is set by \yatm{num_runoff_caps}.
If this exceeds \param{max_caps} in \href{https://github.com/COSIMA/libaccessom2/blob/91cb1437b9952d1bb20b6f5b833fceaa9241fe87/atm/src/runoff.F90#L24-L35}{\param{atm/src/runoff.F90}}
you'll need to increase \param{max_caps} and add new default values for the extra caps and indices (use 0.0 for the new \param{runoff_caps} elements), then recompile.
The 0.25$^\circ$ and 1$^\circ$ configurations use the default runoff cap of \SI{0.03}{kg.m^{-2} s^{-1}} globally. At 0.1$^\circ$ there are reduced caps \SI{0.003}{kg.m^{-2} s^{-1}} \citep[and also \SI{0.001}{kg.m^{-2} s^{-1}} in the][configuration]{KissETAL2020a} at the mouths of some Arctic rivers to produce broader spreading and avoid excessively low salinity, and a cap of \SI{0.03}{kg.m^{-2} s^{-1}} everywhere else.
...
\subsection{Initial conditions and spinup}\label{S:initial_conditions}
...
The temperature and salinity initial condition file \param{ocean_temp_salt.res.nc} is created by \url{https://github.com/COSIMA/initial_conditions_access-om2}.
%the script \paramsty{/g/data/hh5/tmp/cosima/observations/postprocessing/woa13/tools/setup_WOA_initial_conditions.py}.
...
T and S data are interpolated horizontally into the continents at each depth level so there are no issues with uninitialised wet cells if the model bathymetry differs from WOA.
...
This sets a uniform ice concentration and thickness (100\% and around 2.5\,m, respectively) in regions north of $70^\circ$N and south of $60^\circ$S (see \url{https://github.com/COSIMA/cice5/blob/5583ce54fd8822c1b8aef0549090167ca5f36d10/source/ice_init.F90#L1503-L1505}) where the sea surface temperature is less than or equal to $1^\circ$C above freezing (this is an AusCOM customisation ---- see \url{https://github.com/COSIMA/cice5/blob/5583ce54fd8822c1b8aef0549090167ca5f36d10/source/ice_init.F90#L1601-L1620} and \param{aice} and \param{hi} in \paramsty{/g/data/hh5/tmp/cosima/access-om2-01/01deg_jra55v13_ryf8485_spinup6/output000/ice/OUTPUT/iceh.0001-01-01.nc}).
The initial SST comes from the input file \param{monthly_sstsss.nc} --- see \url{https://github.com/COSIMA/cice5/blob/5583ce54fd8822c1b8aef0549090167ca5f36d10/drivers/auscom/CICE_InitMod.F90#L159}.
...
cf. Matt Chamberlain's 2016 talk: global MOM-SIS at 0.1$^\circ$ and 50 levels, 960 CPUs (50x23 layout, 200 masked), dt=720s, month $\sim$100min: \url{https://cosima.org.au/wp-content/uploads/2016/06/ofam_global.mac_.pdf} --- this is as fast as ACCESS-OM2-01 but about 6x cheaper!
...
cf. oceanMAPS3.0  \url{https://cosima.org.au/wp-content/uploads/2016/06/Brassington_Ocean_modelling_and_forecasting_v3.pptx.pdf} % /Users/andy/Documents/COSIMA/COSIMA workshop 26-27 May 2016/Gary Brassington (BoM) Brassington_Ocean_modelling_and_forecasting_v3.pptx.pdf
...
ACCESS-OM2 uses the same MOM, CICE and OASIS versions as ACCESS-CM2 (1$^\circ$) \citet{BiYanSullivan2016a}, \url{https://cosima.org.au/wp-content/uploads/2016/06/BI-COSIMA-Hobart-20160526.ppt.pdf}
...
It is the depth at which the buoyancy exceeds the surface buoyancy by more than the default buoyancy criterion \mom{buoyancy_crit}=\SI{0.0003}{m s^{-2}} as recommended for OMIP \citep[][appendix H24]{GriffiesETAL2016a}.
Since buoyancy is $g\Delta\sigma/\rho$, this is very nearly identical to the $\Delta\sigma=\SI{0.03}{kg m^{-3}}$ density criterion used by \citet{SalleeShuckburghBruneauMeijersBracegirdleWang2013a} and advocated by \citet{DownesBindoffRintoul2009a, SalleeWiendersSpeerMorrow2006a} and \citet{Boyer-MontegutETAL2004a}.
\param{mld} is calculated and \href{https://github.com/mom-ocean/MOM5/blob/6fb47f6/src/mom5/ocean_diag/ocean_tracer_diag.F90#L1049-L1142}{linearly interpolated between grid levels} every timestep. If the mixed layer reaches the bottom of the ocean, the bottom depth is returned.
...
\subsection{At 0.1 degree}\label{0.1changes}
\begin{itemize}
...
\item Use explicit background vertical diffusivity, spatially uniform (\mom{j09_bgmax}=\mom{j09_bgmin}=$\SI{1e-6}{m^2s^{-1}}$).
...
See \url{https://github.com/COSIMA/access-om2/issues/134} and \url{https://github.com/rmholmes/cosima-scripts/blob/master/Equatorial_Pacific_Analysis/Equatorial_Pacific_Thermal_Bias.ipynb}
\end{itemize}
...
Model development has been partly funded by ARC Linkage grants LP160100073 and LP200100406.
