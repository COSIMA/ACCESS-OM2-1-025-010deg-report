...
\end{center}
    ...
    Coupling between model components by OASIS3-MCT-2 as specified in the \param{namcouple} file.
    \TODO{see \param{forcing.json} and input fields in \mom{mom_oasis3_interface_nml} in \param{input.nml}}
...
See \paramsty{/short/v45/aek156/access-om2/input/mom_01deg/README-topog.txt}.
...
See \url{https://github.com/OceansAus/access-om2/issues/99} --- \WARNING this produces terraces in shallow water; this is fixed in later versions.
...
\param{tideamp.nc} probably originates from \citet{JayneLaurent2001a}.\TODO{try to confirm this?}
...
Tidal current speed is read from the file \param{tideamp.nc} \param{tideamp.nc} \citep[probably from][]{JayneLaurent2001a} and bottom roughness is read from the file \param{roughness_cdbot.nc}.
The bottom drag coefficient is restricted to the range \mom{cdbot_lo} to \mom{cdbot_hi}, where \mom{cdbot_lo} takes its default value 0.001 and \mom{cdbot_hi}=0.007 exceeds the default value 0.003.
\mom{cdbot} is unused, since \mom{cdbot_law_of_wall}=false (the default).

\paramsty{/short/public/access-om2/input_38570c62/mom_01deg/}\param{tideamp.nc}
\paramsty{/short/public/access-om2/input_38570c62/mom_01deg/}\param{roughness_amp.nc}
\paramsty{/short/public/access-om2/input_38570c62/mom_01deg/}\param{roughness_cdbot.nc}
and \param{drag_coeff} in \param{ocean_grid.nc}
% \TODO{where does the tidal current data come from?} % see https://arccss.slack.com/archives/C9Q7Y1400/p1554428974011900
% Russ says
%tideamp is probably from Jayne and Laurent 2001 if it came from GFDL.

\TODO{where does the roughness data come from? These seem prettymuch the same, except \param{roughness_amp.nc} is on T points and \param{roughness_cdbot.nc} is on U points}
...
For flux fields a 2nd order conservative remapping method is used \FIXME{not at 0.1 deg} and for scalar fields we use a technique called patch recovery \citep{Kritsikis2017, Khoei2007}. 
...
Initial conditions were from spun-up runs \TODO{in what month?}
...
\item TEOS-10? - requires conservative temp and absolute salinity; isn't more expensive than pre-TEOS-10 - \url{https://github.com/OceansAus/access-om2/issues/140}
...
These tables are auto-generated by \paramsty{namelists/make\_tables.py} which uses nmltab (\url{https://github.com/aekiss/nmltab}).
...
These are auto-generated by \paramsty{namelists/make\_tables.py} which uses nmltab (\url{https://github.com/aekiss/nmltab}). 
Variables are weblinks to source code searches. 
