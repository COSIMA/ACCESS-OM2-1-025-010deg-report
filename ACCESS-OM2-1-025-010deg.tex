...
The sea-ice component in the ACCESS-OM2 suite is the LANL CICE5.1.
...
5 thickness categories. \TODO{where is this specified?}
...
% See ``Fast EVP solutions in a high-resolution sea ice model'' Nikolay V. Koldunov et al - Andy email 23/1/2019
...
%At 0.25$^\circ$ and 0.1$^\circ$ 
In all three configurations the vertical grid resolution is fine enough to resolve the surface Ekman layer (Table~\ref{T:vgrid}), so we use a turning angle of zero (\cice{cosw}=1.0, \cice{sinw}=0.0).
...
4 ice layers + 1 snow \TODO{where is this specified?}
...
0.1 deg run affected by forcing bug: \url{https://github.com/marshallward/payu/issues/138}
