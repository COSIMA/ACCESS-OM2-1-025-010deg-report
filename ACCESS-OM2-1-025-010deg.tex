...
\newcommand{\mom}[1]{\param{\momlink{#1}{#1}}\index{\param{#1}}}
\newcommand{\cice}[1]{\param{\cicelink{#1}{#1}}\index{\param{#1}}}
\newcommand{\yatm}[1]{\param{\yatmlink{#1}{#1}}\index{\param{#1}}}
\newcommand{\accessom}[1]{\param{\accessomlink{#1}{#1}}\index{\param{#1}}}
...
At all three resolutions tracers (salt, temperature and age) are advected horizontally and vertically by the multidimensional piecewise parabolic method \citep{ColellaWoodward1984a}, with a monotonicity-preserving flux limiter following \citet{SureshHuynh1997a} (\param{horizontal-advection-scheme}=\param{vertical-advection-scheme}='mdppm' and \param{ppm_hlimiter}=\param{ppm_vlimiter}=3 for temp, salt and age in \param{ocean/field_table}; also see \url{https://github.com/mom-ocean/MOM5/blob/master/src/mom5/ocean_tracers/ocean_tracer_advect.F90}).
...
Typical computational resource requirements for the three configurations.
...
In CICE we use \cice{distribution_type}=`roundrobin` with \cice{processor_shape}=`square-ice` at $0.25^\circ$ and $0.1^\circ$ \citep{CraigMickelsonHunkeBailey2014a}, which omits land-only blocks and also improves the load balance.
...
These tables are auto-generated by \param{namelists/make\_tables.py} which uses nmltab (\url{https://github.com/aekiss/nmltab}).
...
These are auto-generated by \param{namelists/make\_tables.py} which uses nmltab (\url{https://github.com/aekiss/nmltab}). 
Variables are weblinks to source code searches. 
