...
\newcommand{\ISSUE}[1]{\note{\colorbox{yellow}{\textsf{\textbf{\href{https://github.com/OceansAus/ACCESS-OM2-1-025-010deg-report/issues/#1}{ISSUE #1}}}}}\index{ISSUE}}
\newcommand{\CISSUE}[1]{\note{\colorbox{gray}{\textsf{\textbf{\href{https://github.com/OceansAus/ACCESS-OM2-1-025-010deg-report/issues/#1}{ISSUE #1}}}}}\index{ISSUE (closed)}}
...
\newcommand{\nmllink}[2]{#1\index{\paramsty{#1}}} % don't link variables
...
%%\renewcommand{\nmllink}[2]{\href{https://github.com/OceansAus/cice5/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
...
%%\renewcommand{\nmllink}[2]{\href{https://github.com/OceansAus/matm/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
...
\end{center}
    ...
    Coupling between model components by OASIS3-MCT-2 as specified in the \param{namcouple} file (which matches the atmosphere-CICE coupling fields specified in \param{forcing.json} and the CICE-MOM coupling fields specified in \mom{mom_oasis3_interface_nml} in \param{input.nml}).
    ...
    Also see \url{https://github.com/OceansAus/access-om2/wiki/System-description}.
}
    \label{F:coupling}
\end{figure}
...
The final source code and run configurations used here are at \url{https://doi.org/10.5281/zenodo.2637649} (or equivalently at \url{https://github.com/OceansAus/access-om2/releases/tag/GMD2019}).
The individual final run configurations are at
\url{https://github.com/OceansAus/1deg_jra55_iaf/releases/tag/1.0},
\url{https://github.com/OceansAus/025deg_jra55_iaf/releases/tag/1.0} and
\url{https://github.com/OceansAus/01deg_jra55_iaf/releases/tag/1.0}
}
\label{T:configs}
\end{table}
...
The vertical grids are specified in the file \param{ocean_vgrid.nc} for each configuration.
...
Various iterations of the 0.1$^\circ$ bathymetry are in \paramsty{/g/data3/hh5/tmp/cosima/bathymetry}; see \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/README} and \paramsty{/short/v45/aek156/access-om2/input/mom_01deg/README-topog.txt}.

\param{kmt_min}: minimum number of vertical cells

The \param{topog.nc} seafloor topography (bathymetry) files were set up at each resolution with the help of \url{https://github.com/OceansAus/topogtools} and \url{https://github.com/mom-ocean/MOM5/blob/master/src/tools/make_topog/topog.c} and are read in by \url{https://github.com/mom-ocean/MOM5/blob/master/src/mom5/ocean_core/ocean_topog.F90} at runtime.
This reads \paramsty{work/ocean/INPUT/topog.nc} which points to \paramsty{/short/v45/aek156/access-om2/input/mom_01deg/topog.nc} - OR DOES IT?  This in turn is the same as \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/topog_13_06_2018.baffin.nc}.
At $0.1^\circ$ \paramsty{/short/v45/aek156/access-om2/control/01deg_jra55_iaf/config.yaml} specifies \paramsty{/short/public/access-om2/input_38570c62/mom_01deg/topog.nc}, which is the same as \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/topog_13_06_2018.baffin.nc}.
...
Mention the integrity checks and scripts used to generate the data (e.g. in \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/tools/common/}) --- should these be made publicly available? Also  \url{https://github.com/aekiss/notebooks/blob/master/non-advective.ipynb} -- run this check at lower resolution also?
...
See \url{https://github.com/OceansAus/access-om2/issues/141}.
...
The topo data used in the runs is \paramsty{/short/v45/aek156/access-om2/input/mom_01deg/}\param{topog.nc}
also \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/}\param{topog_latest.nc}
\FIXME{update}
...
\paramsty{/g/data3/hh5/tmp/cosima/bathymetry/README}: ``Enforced minimum of 7 levels (approx 10m). Excavated not filled in so land mask kept. Partial cells: Enforced thickness of max(10,0.2*dz). If partial cell were thinner than half this then the cell was removed.''
...
\WARNING This means the minimum thickness is greater than 20\% of the full cell thickness if this is less than 50\,m (i.e.\ for depths shallower than 543\,m) and the cells are full-depth if they thinner than 10\,m  (i.e.\ for depths shallower than 103\,m) --- see figure~\ref{F:partialcells} and \url{https://github.com/OceansAus/access-om2/issues/99}). %See figures/bathymetry/bathymetry.ipynb
...
Turbulent mixing and viscosity due to breaking internal tides is parameterised by the \citet{SimmonsJayneLaurentWeaver2004a} scheme (\mom{use_wave_dissipation}=true) with a vertical decay scale \mom{decay_scale}=500\,m, zero background diffusivity (\mom{background_diffusivity}=0.0), a constant background vertical viscosity of \mom{background_viscosity}=\SI{1e-4}{m^2s^{-1}}, tidal current speed read from the file \param{tideamp.nc} (\mom{read_tide_speed}) and bottom roughness amplitude read from the file \param{roughness_amp.nc} (\mom{read_roughness}=true and \mom{reading_roughness_amp}=true).
...
Tidal current speed is read from the file \param{tideamp.nc} \param{tideamp.nc} and bottom roughness is read from the file \param{roughness_cdbot.nc}.
...
The prognostic temperature variable is conservative temperature in the $1^\circ$ and $0.25^\circ$ configurations (\mom{temperature_variable}='conservative_temp'), and \WARNING potential temperature\footnote{This was an oversight: \url{https://github.com/OceansAus/access-om2/issues/97}, and has been corrected in the new $0.1^\circ$ RYF configuration, which uses relative velocities for both ice and ocean: \url{https://github.com/OceansAus/access-om2/projects/1}.} at $0.1^\circ$ (\mom{temperature_variable}='potential_temp').
...
The MOM baroclinic timestep is \accessom{ice_ocean_timestep} in \param{accessom2.nml}.
This is also the CICE thermodynamic timestep.
...
Commit 076b14f2 did not include several bug fixes \WARNING which we have since imported from the CICE consortium (see commits cbf4c62 through to 320a796 at \url{https://github.com/OceansAus/cice5/commits/master}).
...
The entire $0.1^\circ$ deg run was affected by a forcing bug: \WARNING \url{https://github.com/marshallward/payu/issues/138} and \url{https://github.com/OceansAus/access-om2/issues/123}.
...
This limit is unlikely to have a noticeable effect, since even the slowest-decaying component (clear water in the blue/green band) has a decay scale $k=0.0232$\,m$^{-1}$, so the neglected fraction is at most $\frac{1}{2}\exp(-k$\,\mom{zmax_pen}$)=\SI{4.7e-4}{}$, and probably very much less.
...
River discharge is inserted into the top \mom{river_insertion_thickness}=40\,m, but other runoff is inserted only at the surface (\mom{runoff_insertion_thickness} and \mom{calving_insertion_thickness} default to zero).
...
In order to avoid excessively large fluxes the SSS mismatch is limited to \mom{max_delta_salinity_restore} = 0.5\,psu for the purposes of calculating the restoring flux.
We don't use \mom{salinity_restore_limit_lower} or \mom{salinity_restore_limit_upper}, so \mom{max_delta_salinity_restore} applies even in regions of extremely low or high salinity.
...
We used \href{https://github.com/OceansAus/cice5/search?q=n_itts}{\param{n_itts}}=2 Monin-Obukhov iterations, which \citet{LargeYeager2004a} state is appropriate for over the ocean, but less than their suggested value of 5 for over sea ice. \FIXME{or is this only applied over the ocean?}
...
We use relative wind over ocean: \url{https://github.com/OceansAus/cice5/blob/master/drivers/auscom/surface_flux_mod.F90#L558} - relative to ice as well as ocean? what happens when ice coverage is not 0\% or 100\%?
see \url{https://arccss.slack.com/archives/C9Q7Y1400/p1549506700469900}
Relative winds are used over ocean, but absolute winds over ice: \url{https://github.com/OceansAus/access-om2/issues/138}.
...
There is now an option to use absolute winds to force the ocean via the boolean \mom{absolute_wind} in namelist \mom{surface_flux_nml} --- see \url{https://github.com/OceansAus/access-om2/issues/137}.
...
This is implemented via an associated library (libaccessom2, \url{https://github.com/OceansAus/libaccessom2}) that is linked into YATM, CICE and MOM to provide shared functionality and an interface to inter-model communication and synchronisation tasks. 

YATM is also responsible for remapping river runoff in real-time. This is done separately from the other forcing fields because it is difficult to do in a distributed memory setting --- ensuring runoff is on coastal points may require interprocess communication. remapping is done in two steps: firstly it is moved to the destination grid using conservative interpolation, and then distributed from land to coastal points using an efficient nearest neighbour algorithm based on a pre-computed k-dimensional tree (k-d tree) data structure \citep{Bentley1975}. The Fortran k-d tree implementation is called KDTREE2 (\url{https://github.com/jmhodges/kdtree2}).
...
The ACCESS-OM2 and ACCESS-OM2-025 runs were started at rest, with zero sea level, with temperature and salinity from World Ocean Atlas 2013 v2 \citep[][ \url{https://www.nodc.noaa.gov/OC5/woa13/}]{LocarniniETAL2013a, ZwengETAL2013a} and run for five 60-year cycles (1 Jan 1958 -- 31 Dec 2017) of JRA55-do.
...
We use $0.25^\circ$ data, monthly climatologies above 1500\,m and seasonal climatologies below that: \paramsty{/g/data3/hh5/tmp/cosima/observations/postprocessing/woa13/tools/setup_WOA_initial_conditions.py}.
...
The maximum number of blocks per CPU is given by the environment variable \param{MXBLCKS} which is calculated from the CPU count (\param{NTASK}) by the \param{build.sh} compilation script.
%At $0.1^\circ$ we use 9000 blocks of size \param{BLCKX}$\times$\param{BLXKY} = $36\times30$, distributed across \param{NTASK}=1392 CPUs (\url{https://github.com/OceansAus/cice5/blob/076b14f284d40f9731f14534e0d2bae3ba3f5fc0/bld/config.nci.auscom.3600x2700}).
...
The profiling runs were nearly identical to the final configurations of the production runs (Appendix~\ref{S:namelists}) but they did have a few differences which are tabulated in Appendix~\ref{S:namelist-prof-diffs}.
We note here the main differences that may affect runtime.
\begin{itemize}
\item The profiled $1^\circ$ runs used Sandy Bridge nodes, whereas the production runs used the faster Broadwell nodes.
...
\item TEOS-10? - requires conservative temp and absolute salinity; doing it properly requires an additional salt tracer, but nobody does it that way. It isn't more expensive than pre-TEOS-10 - \url{https://github.com/OceansAus/access-om2/issues/140}
...
\item Fix up Rayleigh damping locations, especially at 1 degree: section~\ref{S:Rayleigh}
\item increase \cice{ndte}? Hunke said CICE consortium now uses ndte=240 instead of 120 - better for high resolution
\item Set \mom{ncar_boundary_scaling}=false at 0.25 and 0.1deg, since we're not using it anyway
\item Set \mom{zmax_pen} to 1e6? see comments in code
\end{itemize}
...
These tables are auto-generated by \paramsty{namelists/}\param{make\_tables.py} which uses \param{nmltab} (\url{https://github.com/aekiss/nmltab}).
...
%\renewcommand{\nmllink}[2]{\href{https://github.com/mom-ocean/MOM5/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
%\renewcommand{\nmllink}[2]{\hypertarget{mom:#2}{\href{https://github.com/mom-ocean/MOM5/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{accessom2:#2}{\href{https://github.com/OceansAus/libaccessom2/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\nml{accessom2_nml.tex}

\subsection{MOM namelist `input.nml'}\label{S:mom-namelist}
%\renewcommand{\nmllink}[2]{\href{https://github.com/mom-ocean/MOM5/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
%\renewcommand{\nmllink}[2]{\hypertarget{mom:#2}{\href{https://github.com/mom-ocean/MOM5/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{mom:#2}{\href{https://github.com/mom-ocean/MOM5/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\nml{input_nml.tex}

\subsection{CICE namelists}\label{S:cice-namelist}
See \url{https://ncar.github.io/CICE/users_guide/ice_nml_var.html}.
%\renewcommand{\nmllink}[2]{\href{https://github.com/OceansAus/cice5/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{cice:#2}{\href{https://github.com/OceansAus/cice5/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
...
%\renewcommand{\nmllink}[2]{\href{https://github.com/OceansAus/libaccessom2/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{yatm:#2}{\href{https://github.com/OceansAus/libaccessom2/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\nml{atm_nml.tex}
...
These are auto-generated by \paramsty{namelists/}\param{make\_tables.py} which uses \param{nmltab} (\url{https://github.com/aekiss/nmltab}). 
Variables are weblinks to source code searches. 
...
\subsection{YATM namelist `atm.nml'}\label{S:yatm-namelist-prof-diff}
\renewcommand{\nmllink}[2]{\href{https://github.com/OceansAus/libaccessom2/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
