...
\end{itemize}
\item check that sync_output_to_gdata.sh and RM_SYNCHED_OUTPUT.sh are consistent - or merge into one script with a --remove-source-files option
...
\item check whether \cice{ocn_albedo}=0.1 is appropriate (it is larger than the value 0.06 used in ACCESS-OM and outside the range of \citet{LargeYeager2009a}). Or use \citet{LargeYeager2009a} latitude-dependent ocean albedo (\cice{cst_ocn_albedo}=false)?
\end{itemize}
