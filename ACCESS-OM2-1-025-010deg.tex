...
There are no explicit tides, but bottom-enhanced internal tidal mixing is parameterised by the \citep{Simmons2004} scheme with a vertical decay scale \mom{decay_scale}=500\,m, zero background diffusivity (\mom{background_diffusivity}=0.0) and a constant background vertical viscosity of \mom{background_viscosity}=\SI{1e-4}{m^2s^{-1}}. 
Further details are in the \mom{ocean_vert_tidal_nml} namelist group.
\TODO{Russ' email 1 March: There's also the Lee et al scheme for coastal barotropic effects that is used (at least in the 0.1 model). Lee, Rosati and Spelman, 2003. Ocean Modelling 11, 464-477}

\subsubsection{Rayleigh drag}
Rayleigh drag is a velocity tendency term proportional to the velocity, with a negative coefficient whose magnitude is the $e$-folding time for velocity decay.
It is a momentum sink that is employed as an engineering fix to improve model stability and/or realism in a few selected locations, specified in \param{ocean/field_table}.
...
The ACCESS-OM2 and ACCESS-OM2-025 runs were started from World Ocean Atlas 2013 v2 \url{https://www.nodc.noaa.gov/OC5/woa13/} and run for five 60-year cycles (1 Jan 1958 -- 31 Dec 2017) of JRA55-do.

The ACCESS-OM2-01 experiment \param{01deg_jra55v13_iaf} ran for 33 years from 1 Jan 1985 -- 31 Dec 2017. 
It was started from restart 431 from \param{01deg_jra55v13_ryf8485_spinup6}, i.e.\ after 40 years of 1 May 1984 -- 30 April 1985 repeat-year forcing, which in turn began from a World Ocean Atlas 2013 v2 initial condition on 1 January.
