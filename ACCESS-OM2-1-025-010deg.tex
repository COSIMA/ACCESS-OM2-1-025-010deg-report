...
The model components are coupled together via Ocean Atmosphere Sea Ice Soil (OASIS3-MCT) version 2.0 from CERFACS and CNRS, France (\url{https://portal.enes.org/oasis}). The exact source code and inputs used for the experiments discussed here are listed in table~\ref{T:configs}.

\begin{table}[tbp]
% update this with
%      cd namelists; python make_tables.py
\input{namelists/configurations.tex} 
\caption{Sources and NCI paths to executables, inputs and outputs for the experiments in this document. These are based on the final run of each experiment; consult run summary spreadsheets for information on any changes within these experiments.}\label{T:configs}
\end{table}
...
The models use a $z^*$ vertical coordinate \citep[][section~5.1.4]{Griffies2012a}, with partial cells (see section~\ref{S:bathymetry}).
...
ACCESS-OM2 		&	 50 	&	 10.0	&	85.0	&	334.7	&	6000.0 \\
...
\hline
\end{tabularx}
\caption{Vertical grid parameters: $n$ levels, with spacing of $\Delta z_\text{min}$ and $\Delta z_\text{max}$ at the surface and maximum depth $H_\text{max}$, respectively, and median spacing $\Delta z_\text{median}$.\TODO{check that I'm correctly using the notation in \citet{StewartHoggGriffiesHeerdegenWardSpenceEngland2017a}}}\label{T:vgrid}
...
\caption{Vertical grid spacing for the ACCESS-OM2 simulations. The horizontal lines show the top and bottom of T cells.}
\label{F:vgrid}
\end{figure}
...
The grid remapping interpolation weights are calculated using the RegridWeightGen application which is part of the ESMF framework \url{https://www.earthsystemcog.org/projects/esmf/}. For flux fields a 2nd order conservative \url{https://www.geosci-model-dev.net/10/425/2017/gmd-10-425-2017.bib} regridding method is used and for scalar fields we use a technique called patch recovery \url{https://dl.acm.org/citation.cfm?id=1238382}. River runoff is regridded separately to the other fields. In addition to conservative interpolion river runoff is also redistributed from land to coastal points using an efficient nearest neighbour algorithm based on a pre-computed k-dimensional tree data structure \url{https://dl.acm.org/citation.cfm?doid=361002.361007}. 
...
Some (all?) runs affected by forcing bug: \url{https://github.com/marshallward/payu/issues/138} - which payu versions are affected?
