...
\item It forms the basis of one or more journal papers to announce and assess the performance of these models, e.g.\ \citet{KissETAL2019a}.
...
Model development is public (\url{https://github.com/COSIMA/access-om2}) and all model code, configuration files and inputs are available to download and ready to run on NCI's Raijin supercomputer.
...
The ACCESS-OM2 model suite is described by \citet{KissETAL2019a}; additional technical details are provided here.
...
Various iterations of the 0.1$^\circ$ bathymetry are in \paramsty{/g/data3/hh5/tmp/cosima/bathymetry}; see \paramsty{/g/data3/hh5/tmp/cosima/bathymetry/README} and \paramsty{/short/v45/aek156/access-om2/input/mom_01deg/README-topog.txt} and \paramsty{/home/156/aek156/payu/01deg_jra55v13_ryf8485_spinup6/README-topog.txt}.
...
\TODO{check - we have \mom{use_side_drag_friction} unspecified and the default is false}
...
Frazil production is not confined to the top model layer (\mom{frazil_only_in_surface}=false) and uses the pre-TEOS-10 freezing temperature (\mom{freezing_temp_preteos10})=true.
...
Downslope transport and mixing schemes are only used at $1^\circ$, where we have sigma transport \citep{BeckmannDoscher1997a, CampinGoosse1999a, DoscherBeckmann2000a} with default parameters (\mom{ocean_sigma_transport_nml}) and downslope mixing (\mom{ocean_mixdownslope_nml}) with \mom{mixdownslope_npts}=4.
...
\accessom{ice_ocean_timestep} is chosen to be a factor of the JRA55-do forcing period of 3\,hr = 10800\,s, for example one of 200, 216, 225, 240, 270, 300, 360, 400, 432, 450, 540, 600, 675, 720, 900, 1080, 1200, 1350, 1800, 2160, 2700, 3600 or 5400\,s.
...
\paramsty{/g/data/qv56/replicas/input4MIPs/CMIP6/OMIP/MRI/} (preferred) and also
\paramsty{/g/data1/ua8/JRA55-do/RYF/v1-3/*.nc} or equivalently
\paramsty{/g/data/ua8/JRA55-do/latest/}.
Also see \url{http://climate-cms.unsw.wikispaces.net/JRA55-do} and \url{https://github.com/COSIMA/access-om2/issues/120}.

The JRA55-do forcing file paths are specified in \paramsty{atmosphere/}\param{forcing.json}.
...
RYF: cite \citep{StewartKimUrakawaHoggYeagerTsujinoNakanoKissDanabasoglu2019a}; cf.\ \citet{Roske2006a}.
...
We impose a constraint of zero net water flux into the ocean from the coupler (\mom{zero_net_water_coupler} = true) by removing the area-mean of P-E+R from P-E so that the area-integrated P-E+R=0 at each timestep; this does not constrain water exchanges with the sea ice (see section~\ref{S:oceanvol}).
...
We use relative wind over ocean: \url{https://github.com/COSIMA/cice5/blob/master/drivers/auscom/surface_flux_mod.F90#L558} - relative to ice as well as ocean? what happens when ice coverage is not 0\% or 100\%?
...
The number of CICE cores actually used in a run is specified via \param{ncpus} in the ice section of \param{config.yaml}, which must equal \cice{nprocs} in \param{cice_in.nml}.
...
(\param{NTASK}) by the \param{build.sh} compilation script.
...
\citet{KissETAL2019a} present evaluations of the ACCESS-OM2 model suite; additional details are provided here.

Data from all experiments are stored on NCI in \texttt{/g/data3/hh5/tmp/cosima} and can be accessed and analysed via the COSIMA Cookbook (\url{https://github.com/COSIMA/cosima-cookbook}).
...
AMOC reviews: \citet{ZhangSuttonDanabasogluKwonMarshYeagerAmrheinLittle2019a}, \citet{FrajkaWilliamsETAL2019a} and \citet{HirschiETAL2019b}.
...
\item turn off MOM output/restart compression (since this can be done for both output and restart by payu in collation step, obviating the need for mppnccombine-fast): set \mom{deflate_level=-1} \url{https://github.com/COSIMA/access-om2/issues/168}
\end{itemize}
...
\item Use improved bathymetry that
    \begin{itemize}
        \item Sets min_thick to the top level thickness in bathymetry: \url{https://github.com/COSIMA/access-om2/issues/99}
    ...
    \end{itemize}
\item Fix salt restoring file \url{https://github.com/COSIMA/access-om2/issues/74}
\item Use explicit vertical diffusivity (e.g.\ \mom{background_diffusivity} = $\SI{1e-6}{m^2s^{-1}}$, spatially uniform, or $\SI{1e-5}{m^2s^{-1}}$ in mid-latitudes and $\SI{1e-6}{m^2s^{-1}}$ in tropics --- see Ryan's MOM-SIS experiments; also vertical variation?) --- may improve (thicken) thermocline in equatorial Pacific and Indonesian seas? see \url{https://github.com/COSIMA/access-om2/issues/134}
\end{itemize}
...
\begin{itemize}
    ...
    \item output bih_fric_u and bih_fric_v --- we can take the curl in post-processing
    ...
    \item fix units for temp: \url{https://github.com/mom-ocean/MOM5/issues/295}
    \item output daily min and max SST as well as mean --- min is useful for comparison with foundation temperature or night time obs.
...
\end{itemize}
\item move inputs from /short/public to project ik11 for transition to gadi
\item fix PBS directives for gadi \url{https://opus.nci.org.au/display/Help/Preparing+for+Gadi}
\item update compile scripts for gadi --- library versions, compiler flags
\item merge sync_output_to_gdata.sh and RM_SYNCHED_OUTPUT.sh  into one script with a --remove-source-files option
...
\item Compile with intel 19 and openMPI3.0.3? \url{https://github.com/COSIMA/access-om2/issues/127} --- though Marshall has had difficulty with it \url{https://arccss.slack.com/archives/C9Q7Y1400/p1547103977041900} 
\item Use WOA2018 for initial condition and salt restoring? \url{https://www.nodc.noaa.gov/OC5/woa18/} --- like WOA13v2, WOA2018 has discontinuities and noise in Arctic surface salinity \url{https://github.com/COSIMA/access-om2/issues/103}
\item switch to JRA55-do v1.4? - it is required for CMIP6 OMIP-BGC - see Marsland email 2019-08-08 and \url{https://github.com/COSIMA/access-om2/issues/155}
\item use new JRA55-do path? \url{https://github.com/COSIMA/access-om2/issues/120} - would require membership of qv56 via \url{https://my.nci.org.au/mancini/project/} for all users. Can YATM handle this file format?
...
\item TEOS-10? - requires conservative temp and absolute salinity; doing it properly requires an additional salt tracer, but nobody does it that way. It isn't more expensive than pre-TEOS-10 - \url{https://github.com/COSIMA/access-om2/issues/140}
\item Investigate wavy ice features in 0.25deg --- poor EVP convergence? \url{https://github.com/COSIMA/access-om2/issues/87}
\item Stop using 2nd order conservative for 1 and 0.25 deg until we work out how to avoid negative values: \url{https://github.com/COSIMA/access-om2/issues/71}and \url{https://github.com/COSIMA/access-om2/issues/74#issuecomment-454660871}
\item increase \cice{ndte}? Hunke said CICE consortium now uses ndte=240 instead of 120 - better for high resolution
\item don't specify \mom{domains_stack_size} in 1deg? see \url{https://arccss.slack.com/archives/C9Q7Y1400/p1565248895009700}
\item save min and max MLD as well as mean
\item parallel I/O? \citet{YangWardEvans2019a}
\item check whether \cice{ocn_albedo}=0.1 is appropriate (it is larger than the value 0.06 used in ACCESS-OM and outside the range of \citet{LargeYeager2009a}). Or use \citet{LargeYeager2009a} latitude-dependent ocean albedo (\cice{cst_ocn_albedo}=false)? \url{https://github.com/COSIMA/access-om2/issues/172}
...
\item Use conservative temperature (Steve's comment, 6 Dec 2018: it should be OK to simply rename the density variable in the restart if we want to start from previous IAF rather than WOA) \url{https://github.com/COSIMA/access-om2/issues/97}; need to set \mom{pottemp_equal_contemp} to false: \url{https://github.com/COSIMA/access-om2/issues/148}
\item Output vorticity at only one depth
\item Make cice output more compact - compress, and collate daily into monthly \url{https://github.com/COSIMA/cice5/issues/26}
...
\item Distributed iceberg flux, using JRA55-do 1.4.0 solid runoff
