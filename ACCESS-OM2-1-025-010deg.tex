...
The grid covers the global ocean, extending from the North Pole to 81$^\circ$S, % actually -81.1297513555451 at 0.1deg
omitting the Antarctic landmass.
The model longitude range is $-280^\circ$E -- $+80^\circ$E, placing the join in the middle of the Indian Ocean.
...
Horizontal friction is implemented with a biharmonic operator and a Smagorinsky scaling for the viscosity coefficient \citep{Griffies2000a}; see namelist group \mom{ocean_bihgen_friction_nml}.
...
Ice is massless / levitating (\mom{max_ice_thickness}=0) to avoid coupled ice-ocean instabilities, but this alters the pressure applied to the ocean by the sea ice and precludes exact mass conservation in the coupled ocean and sea ice system. \TODO{reference - Griffies / Hallberg?}
...
\url{http://climate-cms.unsw.wikispaces.net/JRA55-do} - data is in /g/data/ua8/JRA55-do/latest/
...
timescale set by \mom{salt_restore_tscale} (we use 10 days in 0.1 deg models, larger in others - see table) but what really matters is piston velocity.
...
Online runoff remapping by YATM via KDTREE2, with spatially variable conservative flux limiting (values set by \yatm{runoff_caps}, $i$-index start and end regions set by \yatm{runoff_caps_is} and \yatm{runoff_caps_ie}
