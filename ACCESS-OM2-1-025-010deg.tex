...
There is no direct coupling from YATM to MOM5. Surface forcing is handled globally by CICE5, which obtains atmospheric data from YATM, interpolates it in time, and calculates various forcing fluxes to pass on to MOM5 via the OASIS3-MCT coupler (figure~\ref{F:coupling}).
...
All versions of JRA55-do up to and including (at least) v1.4.0 had some tropical cyclones in the Northeast Pacific \WARNING and the North Atlantic from 1959 to 1987 erroneously represented as anti-cyclonic vortices: \url{https://github.com/COSIMA/access-om2/issues/186}.
...
These choices are typical of CORE-II models \citep[][table~2]{DanabasogluETAL2014a}.
...
\item dropped support for CORE configs (mark as deprecated) \url{https://arccss.slack.com/archives/C6PP0GU9Y/p1574995599016100}
...
\subsection{For all resolutions}
\begin{itemize}
\item put date in MOM output filenames, and make outputs a consistent length (e.g. monthly at 0.1 deg) \url{https://github.com/COSIMA/access-om2/issues/185}
\item fields for high/low resolution comparison? see Chassignet email 17 Jan 2020
\item fix initial condition \url{https://github.com/COSIMA/access-om2/issues/39}
...
\item Use latest code versions (and make sure configs are compatible with them)
\item use \citet{LargeYeager2009a} latitude-dependent ocean albedo (\cice{cst_ocn_albedo}=false)? \url{https://github.com/COSIMA/access-om2/issues/172}. This would require some testing.
\item Performance: Maybe the -ftz option could be worth trying, in case denormalised floats are hampering performance? We might also want to try -axCORE-AVX512 instead of -axCORE-AVX2 (though this will restrict the executable to Cascade Lake).
...
\end{itemize}
\item specify sensible ncpus and platform: nodesize: 48 in config.yaml - no need to pad total to a multiple of cores per node \url{https://github.com/payu-org/payu/issues/97}
...
\item fix vertical grid issue \url{https://github.com/COSIMA/access-om2/issues/159}
\end{itemize}
...
Easy things:
\begin{itemize}
\item check /short/public/access-om2/input_08022019/mom_01deg/topog.nc --- looks like an old file - has 10m min depth!
