...
\begin{itemize}
\item The profiled $1^\circ$ runs used \mom{redsea_gulfbay_sfix}, but this wasn't used in production runs.
\item The CICE \cice{distribution_type} was inconsistent between profiling runs at both $0.25^\circ$ and $0.1^\circ$, with some using `sectrobin' and some using `roundrobin`; the production runs used `roundrobin` at both resolutions (apart from the first 10 runs at $0.1^\circ$ which used `cartesian`).
