...
\item The CICE5 runs used \cice{distribution_type}=`sectrobin' at both $0.25^\circ$ and $0.1^\circ$, but the production runs used `roundrobin` at both resolutions (apart from the first 114 runs at $0.1^\circ$ which used `cartesian`). 
...
Differences in timestep counters are ignored.
Tables are omitted if there are no differences at all.
\FIXME{exclude cice runs from mom comparison and vice-versa}
\renewcommand{\nmldiffer}[1]{#1} % no special display of differing variables
