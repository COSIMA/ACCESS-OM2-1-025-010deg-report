...
\TODO{copy things from ARCCSS workshop poster, AMOS2018 talk, COSIMA workshop}
% done: Bluelink talk
...
In contrast, the ACCESS-OM2-01 configuration resolves the first baroclinic deformation radius away from shelves and equatorward of about 50$^\circ$\citep{Hallberg2013a}, so resolves the mesoscale in most of the world ocean. It is suitable for runs of several decades.
...
The model components are coupled together via Ocean Atmosphere Sea Ice Soil (OASIS) version 3-MCT from CERFACS and CNRS, France (\url{https://portal.enes.org/oasis}).
...
Figure~\ref{F:horizgrid} shows the grid spacing at the three resolutions.

The grid covers the global ocean, extending from the north pole to 81$^\circ$S, omitting the Antarctic landmass.
The grid is Mercator (i.e.\ meridional spacing scales as the cosine of latitude) between 65$^\circ$N -- 65$^\circ$S, and tripolar \citep{Murray1996a} north of 65$^\circ$N, with tripoles placed on land at 65$^\circ$N and -100$^\circ$E, 80$^\circ$E. % see windrotation.ipynb
\TODO{describe spacing south of 65S}

\TODO{describe grid refinement at equator in 1$^\circ$ model} - is it refined to 1/3$^\circ$ within 10$^\circ$ of Equator? see \citet{BiMarslandUotilaOFarrellFiedlerSullivanGriffiesZhouHirst2013a}
...
\TODO{histogram of grid spacing (in km) and perhaps also maybe also maps of grid spacing divided by local 1st baroclinic Rossby radius from CheltonDeSzoekeSchlaxEl-NaggarSiwertz1998a \url{http://www-po.coas.oregonstate.edu/research/po/research/rossby_radius/index.html}}
...
\caption{Horizontal grid spacing for the ACCESS-OM2 simulations. Note the meridional refinement near the equator in the 1$^\circ$ grid.
...
\paragraph{Sub-grid scale lateral.neutral physics}
...
An along-isopycnal Redi diffusivity \citep{Redi1982} for tracers is also included, with a constant coefficient of $600$m$^2$s$^{-1}$. 
...
All three configurations include a parameterization for restratification in the surface mixed layer due to submesoscale eddies \citep{Fox-Kemper2008}. 
...
Horizontal friction is implemented with a biharmonic operator and a Smagorinsky scaling for the viscosity coefficient \citep{Griffies2000a}. 
...
The suppression of large lateral tracer gradients is thus achieved by the multi-dimensional piece-wise parabolic tracer advection scheme (\citet{Colella1984}, with a monotonicity-preserving flux limiter following \citet{Suresh1997}).
...
Rayleigh drag in ITF

Ice is massless / levitating (max\_ice\_thickness=0) to avoid coupled ice-ocean instabilities, but this alters the pressure applied to the ocean by the sea ice and precludes exact mass conservation in the coupled ocean and sea ice system. \TODO{reference - Griffies / Hallberg?}
...
CICE parameter sensitivities: \citet{UotilaOFarrellMarslandBi2012a, UrregoBlancoUrbanHunkeTurnerJeffery2016a}
...
mushy ice: \citet{TurnerHunkeBitz2013a} - at all resolutions?
Note: we may have a bug in pond scheme with mushy ice? \url{https://github.com/CICE-Consortium/CICE-svn-trunk/releases/tag/cice-5.1.2}
...
Ocean Atmosphere Sea Ice Soil \citep[OASIS, ][]{ValckeCraigCoquart2015a} version 3-MCT, developed at CERFACS and CNRS, France (\url{https://portal.enes.org/oasis}).
...
IAF from start 1958 -- end 2017, 5 repeats

\subsubsection{JRA55-do and repeat-year forcing}
JRA55-do \citep{TsujinoETAL2018a} is a surface-atmospheric reanalysis product derived from the 55-year Japanese reanalysis (JRA55, \url{http://jra.kishou.go.jp/JRA-55/index_en.html}) and intended for driving oceans models. We use JRA55-do version 1.3 to drive our ice and ocean models.

\url{http://climate-cms.unsw.wikispaces.net/JRA55-do}
...
Smoothing at 0.1deg to deal with spurious discontinuities and noise in WOA13v2: \url{https://github.com/OceansAus/access-om2/issues/103}

Resolved issues with ice formation in fresh regions driving salinity below zero. Ice has a fixed bulk salinity \TODO{give variable name and value - is it 5psu?}, which is obtained from the ocean model, but this can drive ocean salinity below zero in very fresh regions. Under these conditions Russ Fiedler's fix borrows salt from the (surface?) ocean at large. \TODO{provide better explanation and github link} \TODO{clarify - is this only in the 0.1deg model?}
...
MATM/YATM parameters for the three model resolutions are tabulated in Appendix~\ref{S:matm-namelist}.\FIXME{update}

SCRIP regridding for OASIS3-MCT does not scale to 0.1$^\circ$% https://github.com/OceansAus/access-om2/wiki/Creating-Remapping-Weights
Use ESMF\_RegridWeightGen instead; also allows smoother conservative interpolation---needed when model is finer than forcing.
Regridding method - \url{https://github.com/OceansAus/access-om2/wiki/Creating-Remapping-Weights}

Online runoff remapping by MATM/YATM via KDTREE2, with spatially variable conservative flux limiting.
...
Data from all experiments are stored on NCI in \texttt{/g/data3/hh5/tmp/cosima} \TODO{update with DOI} and can be accessed and analysed via the COSIMA Cookbook (\url{https://github.com/OceansAus/cosima-cookbook}).
...
\TODO{see notebooks that compare model Pacific equatorial T and u to Johnson et al 2002 obs}
...
\caption{Ice volume in each category at 0.1$^\circ$ resolution. The solid line shows the annual average. \TODO{show all resolutions}}
