...
We don't use overflow or downslope mixing schemes (see \mom{ocean_overflow_nml},  \mom{ocean_overexchange_nml}, \mom{ocean_mixdownslope_nml}), apart from sigma transport \citep{BeckmannDoscher1997a, CampinGoosse1999a, DoscherBeckmann2000a} at $1^\circ$ with default parameters: \mom{ocean_sigma_transport_nml}.
...
It is best not to have \cice{nprocs} far in excess of \param{NTASK}, since this will increase the load imbalance within CICE (since the number of blocks per core decreases), and is also memory-inefficient.
}
...
\item Check that we want \mom{ocean_sigma_transport_nml} at $1^\circ$ and have chosen sensible parameters % see email to Fabio, 2019-05-15
...
\item use new JRA55-do path: \url{https://github.com/COSIMA/access-om2/issues/120}
...
\item Stop using 2nd order conservative for 1 and 0.25 deg until we work out how to avoid negative values: \url{https://github.com/COSIMA/access-om2/issues/71}
...
\item Fix salt restoring file \url{https://github.com/COSIMA/access-om2/issues/74} --- and runoff too?
\item Use 2nd order conservative weights --- but only after we resolve the negative overshoots problem \url{https://github.com/COSIMA/access-om2/issues/71} and \url{https://github.com/COSIMA/access-om2/issues/74#issuecomment-454660871}
...
\item Use conservative temperature (Steve's comment, 6 Dec 2018: it should be OK to simply rename the density variable in the restart if we want to start from previous IAF rather than WOA) \url{https://github.com/COSIMA/access-om2/issues/97}
...
\renewcommand{\nmllink}[2]{\hexhypertarget{accessom2:#2}{\href{https://github.com/COSIMA/libaccessom2/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
...
%\renewcommand{\nmllink}[2]{\href{https://github.com/COSIMA/cice5/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{cice:#2}{\href{https://github.com/COSIMA/cice5/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
...
%\renewcommand{\nmllink}[2]{\href{https://github.com/COSIMA/libaccessom2/search?q=#2}{#1}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\renewcommand{\nmllink}[2]{\hexhypertarget{yatm:#2}{\href{https://github.com/COSIMA/libaccessom2/search?q=#2}{#1}}\index{\paramsty{#1}}} % link to documentation (requires hyperref package)
\nml{atm_nml.tex}
