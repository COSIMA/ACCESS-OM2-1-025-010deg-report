...
\end{center}
    ...
    Coupling between model components by OASIS3-MCT-2 as specified in the namcouple file.
...
%tides. We find ? ' (2ï¿½/(10 km)) is optimal. In principle,
...
%smoothing radius, and k is a free parameter set as k = 2ï¿½/125 km.
...
The prognostic temperature variable is conservative temperature in the $1^\circ$ and $0.25^\circ$ configurations (\mom{temperature_variable}='conservative_temp'), and potential temperature at $0.1^\circ$ (\mom{temperature_variable}='potential_temp').
...
The default regridding method used within OASIS3-MCT-2 (SCRIP \url{https://github.com/SCRIP-Project/SCRIP}) does not scale to 0.1$^\circ$. 
...
For flux fields a 2nd order conservative regridding method is used \FIXME{not at 0.1 deg} and for scalar fields we use a technique called patch recovery \citep{Kritsikis2017, Khoei2007}. 
...
However the 2nd order conservative regridding method produces spurious extrema \WARNING (including nonsensical incorrect signs such as negative runoff or negative downwelling shortwave fluxes): \url{https://github.com/OceansAus/access-om2/issues/74#issuecomment-454660871}.
...
It uses relative ice-atmosphere velocities for calculating stresses, \WARNING so should probably be turned on for consistency, since relative velocity is used for the ocean.
...
\hline
CICE5 block size & $60\times300$ & $30\times27$ & $36\times30$ \\
CICE5 number of blocks & 6 & 1920 & 9000 \\
CICE5 cores (\param{NTASK}) & 24 (6) & 361 (480) & 1600 (1392) \\
...
Timestep (s) & 5400 & 1800 & 450 \\
kCPU hours per model year & 0.1 & 4.7 & 120\\
Walltime hours per model year & 0.3 & 2.6 & 20\\
Memory (Gb) & 83 & 535 & 2693\\
\hline
\end{tabularx}
...
Core count is rounded up to a multiple of the cores per node (24 in normalbw queue, 16 in normal queue).
...
CICE has the ability to subdivide the computational domain horizontally into blocks, and then parallelise by allocating one or more blocks to each CPU.
...
The final CICE configurations of the runs reported here used \cice{distribution_type}=`roundrobin` with \cice{processor_shape}=`square-ice` at $0.25^\circ$ and $0.1^\circ$ \citep{CraigMickelsonHunkeBailey2014a}, which omits land-only blocks and also improves the load balance.
...
\begin{itemize}
\item The profiled $1^\circ$ MOM5 runs used Sandy Bridge nodes, whereas the production runs used the faster Broadwell nodes.
...
\item The CICE5 runs used \cice{distribution_type}=`sectrobin' at both $0.25^\circ$ and $0.1^\circ$, but the production runs used `roundrobin` at both resolutions (apart from the first 10 runs at $0.1^\circ$ which used `cartesian`). 
...
\TODO{fix make\_tables.py to only use *deg for mom namelist and *cice for cice namelists}
...
\item \accessom{ice_ocean_timestep} was 1800\,s and 400\,s at $0.25^\circ$ and $0.1^\circ$ (respectively); these differ from 1350\,s and 450\,s in Appendix~\ref{S:namelists}, but are within the range of the values used in the production runs (Appendix~\ref{S:namelist-diffs}).
\end{itemize}
...
Canadian Meteorological Centre's (CMC) 0.2 degree SST analysis: 1991 Ð 2017:
