...
\TODO{state git hashes of all model components - and how they have changed in model runs - see /g/data3/hh5/tmp/cosima/access-om2-run-summaries}
...
Grid parameters are summarised in Tables~\ref{T:vgrid} and~\ref{T:access-om-ofam3-access-om2}
...
\hline
\end{tabularx}
\caption{Vertical grid parameters: $n$ levels, with spacing of $\Delta z_\text{min}$ and $\Delta z_\text{max}$ at the surface and maximum depth $H_\text{max}$, respectively.\TODO{check that I'm correctly using the notation in \citet{StewartHoggGriffiesHeerdegenWardSpenceEngland2017a}}}\label{T:vgrid}
...
discuss partial cells - specified in topography?

ACCESS-OM2 uses GFDL50 \FIXME{wrong? doesn't match GFDL50 in table 1 of \citet{StewartHoggGriffiesHeerdegenWardSpenceEngland2017a}} 50 levels, 10.0\,m spacing in top 200\,m then increasing smoothly to 334.7\,m by the bottom at 6000\,m.

ACCESS-OM2-025 uses KDS50 \FIXME{wrong? $\Delta z_\text{min}=2.3m$ doesn't match 2.7m for KDS50 in table 1 of \citet{StewartHoggGriffiesHeerdegenWardSpenceEngland2017a}} 50 levels, 2.3\,m spacing at surface, increasing smoothly to 219.6\,m by the bottom at 5363.5\,m.
%See email to Andy 1 Aug 2018 - IAF uses KDS50, whereas the RYF uses GFDL50? - no, now they are both KDS50

ACCESS-OM2-01: KDS75 \TODO{check: maximum spacing and depth slightly different from KDS75} 75 levels, 1.1\,m spacing at surface, increasing smoothly to 198.4\,m by the bottom at 5808.7\,m.

The KDS50 and KDS75 vertical grids are optimised for resolving baroclinic modes \citep{StewartHoggGriffiesHeerdegenWardSpenceEngland2017a}.
...
\TODO{find 1deg control dir - in /short/v45/as7904/ ?}
...
The minimum height of partial cells is 20\% of the full cell height, or 10\, m, whichever is greater (\url{https://github.com/mom-ocean/MOM5/blob/master/src/tools/make_topog/topog.c#L858}).
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.6\textwidth]{figures/viscosity/viscosity_EAC_clim.png}
\includegraphics[width=.6\textwidth]{figures/viscosity/viscosity_GulfStream_clim.png}
\includegraphics[width=.6\textwidth]{figures/viscosity/viscosity_Agulhas_clim.png}
...
Comment on applicability of KPP at our high vertical resolution - see \citet{VanRoekelAdcroftDanabasogluGriffiesKauffmanLargeLevyReichlRingler2018a}
...
At $0.1^\circ$ we also shorten the dynamic timestep by a factor \cice{ndtd}=3 relative to the thermodynamic timestep for numerical stability, as this model includes ocean regions closer to the tripoles where the grid becomes very fine.
...
Coupling is implemented via Ocean Atmosphere Sea Ice Soil \citep[OASIS3-MCT, ][]{ValckeCraigCoquart2015a} version 2.0, developed at CERFACS and CNRS, France (\url{https://portal.enes.org/oasis}).
...
The coupling timestep is \accessom{ice_ocean_timestep} \TODO{check}
...
Ocean surface roughness following Beljaars (1994) (\mom{beljaars}=true). \TODO{get citation}

Ocean albedo is constant (\cice{cst_ocn_albedo}=true), with the value \cice{ocn_albedo}=0.1.
Shortwave penetration is handled by the GFDL scheme (\mom{ocean_shortwave_gfdl_nml}), with maximum penetration of \mom{zmax_pen}=\SI{300}{m} and \citet{ManizzaLe-QuereWatsonBuitenhuis2005a} optics (\mom{optics_manizza}=true) using a prescribed monthly chlorophyll-a climatology (\mom{read_chl}=true) from \param{chl.nc}. 
...
% https://arccss.slack.com/archives/C9Q7Y1400/p1541559974040600 Geothermal heating data - Russ is fairly sure it is this. Hamza, V., R. Cardoso, and C. F. Ponte Neto, 2008: Spherical harmonic analysis of the EarthÕs conductive heat flow. Int. J. Earth Sci., 97, 205Ð226,
...
Some (all?) runs affected by forcing bug: \url{https://github.com/marshallward/payu/issues/138}

There are no tides but there is parameterised internal tidal mixing. 
See \mom{ocean_vert_tidal_nml}.
...
This is 0.11\,m/day at 0.1$^\circ$,  0.0383\,m/day at 0.25$^\circ$ and 0.167\,m/day at 1$^\circ$ due to the differing vertical resolution (see table~\ref{T:vgrid}).
For comparison, 15\,m/300\,days (i.e.\ 0.05\,m/day) is GFDL's standard piston velocity (Griffies, 12 April 2018), so we are restoring more strongly than GFDL at 0.1$^\circ$ and 1$^\circ$.
...
The 0.25$^\circ$ and 1$^\circ$ configurations use the default runoff cap of \SI{0.03}{kg.m^{-2} s^{-1}} globally.
At 0.1$^\circ$ there are reduced caps of 0.001 and \SI{0.003}{kg.m^{-2} s^{-1}} at the mouths of some Arctic rivers to produce broader spreading and avoid excessively low salinity, and a cap of \SI{0.03}{kg.m^{-2} s^{-1}} everywhere else.
...
Typical model computational resource requirements are shown in table~\ref{T:ComputeResources}; further details are provided on NCI here:
\begin{itemize}
\item /g/data/hh5/tmp/cosima/access-om2-run-summaries/run_summary_1deg_jra55v13_iaf_spinup1_A.csv
\item /g/data/hh5/tmp/cosima/access-om2-run-summaries/run_summary_025deg_jra55v13_iaf_gmredi.csv
\item /g/data/hh5/tmp/cosima/access-om2-run-summaries/run_summary_01deg_jra55v13_iaf.csv
\end{itemize}
...
\hline
Model	 & MOM CPUs &	CICE CPUs	& Timestep (s)  & Walltime hours per model year & Memory (Gb) \\
\hline
...
\caption{The specific experiments used for the figures in this document. Input directories are in table~\ref{T:inputs}.}\label{T:exptdata}
\end{table}
...
or PIOMAS? \citet{SchweigerLindsayZhangSteeleSternKwok2011a}, from \url{ftp://pscftp.apl.washington.edu/zhang/PIOMAS/data/v2.1/}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[height=0.3\textheight]{figures/ice_validation/1deg_concentration_map_NH_1990-2014_03_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/025deg_concentration_map_NH_1990-2014_03_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/01deg_concentration_map_NH_1990-2014_03_mean.png}
\end{center}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[height=0.3\textheight]{figures/ice_validation/1deg_concentration_map_NH_1990-2014_09_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/025deg_concentration_map_NH_1990-2014_09_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/01deg_concentration_map_NH_1990-2014_09_mean.png}
\end{center}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[height=0.3\textheight]{figures/ice_validation/1deg_concentration_map_SH_1990-2014_09_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/025deg_concentration_map_SH_1990-2014_09_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/01deg_concentration_map_SH_1990-2014_09_mean.png}
\end{center}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[height=0.3\textheight]{figures/ice_validation/1deg_concentration_map_SH_1990-2014_02_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/025deg_concentration_map_SH_1990-2014_02_mean.png}\\
\includegraphics[height=0.3\textheight]{figures/ice_validation/01deg_concentration_map_SH_1990-2014_02_mean.png}
\end{center}
...
\item Consistent physical parameters: \url{https://github.com/OceansAus/access-om2/issues/121}
