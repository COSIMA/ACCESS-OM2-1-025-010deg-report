...
for tname in ('ice_step',):
    ...
    for res in resolutions:
        timings_fname = 'timings_{}cice.yaml'.format(res)
        print(timings_fname)
    ...
    ax_cpus[0].set_ylim(0.15, 0.4)
    ax_cpus[1].set_ylim(0.01, 0.02)
    ax_cpus[2].set_ylim(0.0005, 0.0025)

    ax_rt.set_ylim(1e-2, 3)
    ax_rt.set_yscale('log')
    ax_rt.set_ylabel('Runtime / step (s)')
    ax_rt.set_title('(a) Main loop runtime')

    ax_cpus[1].set_ylabel(u'CPU × Runtime / step (hr)')
    ax_cpus[0].set_title('(b) CPU hours')

    ax_rt.legend(handles, [u'1°', u'0.25°', u'0.1°'])

    for fmt in formats:
        plt.savefig('cice_{}.{}'.format(tname, fmt),
